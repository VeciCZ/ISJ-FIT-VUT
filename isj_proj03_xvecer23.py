...
def match_permutations(string, words):
    ...
    matching_perms = sorted(list(perms.intersection(words))) # vase reseni
...
def plur2sing(singular, plural):
    ...
    # inversion using zip
    pl2sg = {value:key for (key, value) in sg2pl.items()} # vase reseni
    return pl2sg

def vect2word(word2vect):
    v2w = {tuple(value):key for (key, value) in word2vect.items()} # vase reseni
    return v2w
